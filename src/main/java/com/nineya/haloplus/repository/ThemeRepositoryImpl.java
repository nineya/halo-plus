...
import com.nineya.haloplus.utils.Version;
...
import com.nineya.haloplus.exception.AlreadyExistsException;
...
public class ThemeRepositoryImpl
    ...
    public String getActivatedThemeId() {
        return this.optionRepository.findByKey(PrimaryProperties.THEME.getValue())
            .map(Option::getValue)
                    ...
                    this.currentTheme =
                        this.fetchThemePropertyByThemeId(currentThemeId).orElseGet(() -> {
                            ...
                            throw new ThemeNotFoundException(
                                "Default theme: " + HaloConst.DEFAULT_THEME_ID + " was not found!");
                        });
                }
            }
            if (fallbackTheme.get()) {
                // need set default theme as fallback theme
                setActivatedTheme(HaloConst.DEFAULT_THEME_ID);
            }
        }
    ...
    public ThemeProperty attemptToAdd(ThemeProperty newProperty) {
        ...
        if (!checkThemePropertyCompatibility(newProperty)) {
            throw new ThemeNotSupportException(
                "当前主题仅支持 Halo-Plus " + newProperty.getRequire() + " 及以上的版本");
        }
        ...
        FileUtils.deleteFolderQuietly(targetThemePath);
        ...
        try {
            FileUtils.copyFolder(sourceThemePath, targetThemePath);
        } catch (IOException e) {
            // clear data
            FileUtils.deleteFolderQuietly(targetThemePath);
            ...
            FileUtils.deleteFolderQuietly(sourceThemePath);
        }
