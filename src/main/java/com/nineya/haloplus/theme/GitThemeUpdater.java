...
import static com.nineya.haloplus.theme.ThemeUpdater.backup;
import static com.nineya.haloplus.theme.ThemeUpdater.restore;
...
import com.nineya.haloplus.utils.GitUtils;
...
public class GitThemeUpdater implements ThemeUpdater {
    ...
    public ThemeProperty merge(ThemeProperty oldThemeProperty, ThemeProperty newThemeProperty)
        ...
        try (final var oldGit = Git.init().setDirectory(oldThemePath.toFile()).call()) {
            // 0. commit old repo
            GitUtils.commitAutomatically(oldGit);
            ...
            try (final var ignored = Git.open(newThemePath.toFile())) {
                // remove remote
                GitUtils.removeRemoteIfExists(oldGit, "newTheme");
                ...
                GitUtils.logCommit(rebaseResult.getCurrentCommit());
